...
Observatory\footnote{\url{https://vlo.clarin.eu}} \citep{VLO} and DARIAH in the
SSHOC Open Marketplace\footnote{\url{https://marketplace.sshopencloud.eu/}} .
...
metadata from the source, leveraging various existing metadata formats, and
converting those to a uniform linked open data representation. This data can
then be used to feed catalogues.
...
file or a \texttt{README.md} file which often contains machine-interpretable
badges. Badges are small images often included on top a \texttt{README}
...
We do not harvest any metadata from intermediaries\footnote{that is, from other catalogues,
...
Usage of a manually curated source registry means that, for this project,
...
interpret the term `tool discovery' to also include such functionality. Such a step,
however, can be envisioned as a separate step before our pipeline.
...
project\footnote{\url{https://codemeta.github.io}}. They developed a vocabulary
for describing software source code, extending the schema.org vocabulary and
contributing their efforts back to them. Moreover, the CodeMeta project defines
mappings, called crosswalks, between their vocabulary and many existing
...
file can be kept under version control alongside a tool's source code. 
...
still allows them to provide additional manual metadata. All these options ensure
that developers themselves can choose precisely how much control to exert over
the metadata and harvester.
...
as developed in CLARIAH and CLARIN. In your projects, you can use whatever you
...
like \texttt{encodingFormat} to tie these to MIME-types.

Do note that a describing a full API is explicitly out of scope for our project.
This would describe exactly which function or
web-endpoints take and return what data. Although this too can be
considered metadata, such functionality goes beyond what we consider
...
Initiative\footnote{\url{https://www.openapis.org}} already provide such
functionality for Web APIs. For software libraries there are also
...
\url{https://github.com/proycon/codemeta-harvester}, written in POSIX shell script} fetches all the git
...
\url{https://github.com/proycon/codemetapy}, written in Python} to turn and combine these into a
single codemeta representation. This produces one codemeta JSON-LD file per
input tool. 

All of these together are loaded in our \emph{tool store}. This is
...
(\url{https://github.com/proycon/codemeta2html}, both written in Python).}. The frontend for CLARIAH is
accessible at \url{https://tools.clariah.nl} and shown in 
...
%TODO: two or three paragraphs on the Ineo pipeline and the CMDI/VLO pipeline?
...
\emph{software metadata requirements} \footnote{\url{https://github.com/CLARIAH/clariah-plus/blob/main/requirements/software-metadata-requirements.md}.}.
An example of this is shown in Figure~\ref{fig:validatationreport}.
...
scale of 0 to 5, and visualised as a coloured star rating in our interface.
This evaluation score itself is part of the delivered metadata and something
which both end users as well as other systems can filter on. It may even serve
as a kind of `gamification' element to spur on developers to provide higher
quality
